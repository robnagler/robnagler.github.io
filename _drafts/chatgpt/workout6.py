...
class WorkoutParser:
    ...
    def _parse_workout(self, workout_text):
        ...
        warmup_start = workout_lines.index('GENERAL WARM UP') + 1 if 'GENERAL WARM UP' in workout_lines else None
        strength_power_start = workout_lines.index('STRENGTH/POWER') + 1 if 'STRENGTH/POWER' in workout_lines else None
        cool_down_start = workout_lines.index('GENERAL COOL DOWN') + 1 if 'GENERAL COOL DOWN' in workout_lines else None

        warmup = workout_lines[warmup_start:strength_power_start] if warmup_start is not None else []
        strength_power = workout_lines[strength_power_start:cool_down_start] if strength_power_start is not None else []
        cool_down = workout_lines[cool_down_start:] if cool_down_start is not None else []
