...
amounts of data and lots of weights. Someone asked me last night, how
AI works. They knew about generating the next token, but they didn't
undertand how that could possibly form completely outputs. What drives
the next token?
...
[!https://commons.wikimedia.org/wiki/File:Ant_hill_in_backyard.jpg]
...
together, space and time can create wonderous things.
...
28,380,000,000,000,000,000 instructions per day or 10E18
[todo: supescript] in a second.

For comparison, human brains have about 80 billion neurons, of which
about a billion are active at any one time. This leads to about 100
trillion (10E14 [todo]) spikes (or "instructions") being processed per
second. In rough terms, an LLM processes about 10,000 times faster
than a human brain.
...
trophectoderm cell will form. That happens radomnly.
...
1. Divide up all the numbers in to subsets: (3), (3, 7), (3, 7, 11),
   etc.
...
(2^n [todo]). For example, for one million numbers, there are
10^301,030 [todo subscript] subsets to check. That's a **really** big
number.
...
known as a nodeterministic polynomial (NP) time problems. These types
...
problem is: has someone explained NP time problems in non-theoretical
language? That's the problem I had when I began this section. I used
an LLM to help me do that. I got a **good enough** answer in seconds
from a deterministic computer.

### Summing It Up


The reason this iI did was the computer
program it used was trained using a stochastic process



n LLM is actually
an algorithm that has to complete in seconds.


are much more complex and also NP problems.

,
because they could be solved by a nondeterministic (aka. magical,
non-existent) computer in polynomial time.





No polynomial-time algorithm for this problem is known.

It always finds the correct answer, but in the worst case it examines
an exponential number of subsets (2
n
). No polynomial-time algorithm for this problem is known.

However, it can't be computed for
all possible subsets by a deterministic computer in polynomial time.
...
influenced by many
factors, but perhaps the most important factor is "luck", or what I
like to call random neural firings. It's not one random event, but a
collection of them that causes one solo to be great and another not so
much. Ants also have random neural firings, yielding better and worse
hills.

LLMs also rely on randomness. A neural network itself is not
random. Randomness occurs during training. I'll talk about that more
in a bit. The point of the above is that randomness is critical to
Jazz, LLMs, and DNA.

we can't
imagine. One produces a Jazz solo, another produces a baby, and LLMs
generate tokens. They work, because they take advantage of time,
space, and randomness.

Perhaps you already knew that computers were fast, that we have big
brains, and that cell growth is exponential. However, I think the
the secret sauce is randomness, and for that I have to talk
...
what do ant colonies, DNA, LLMs, and Jazz musicians have in common?
Time and space. **LOTS** of time and space.


then out pops a baby 9


But this morning I came up with
